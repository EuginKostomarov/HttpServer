...
interface CounterpartiesTabContentProps {
  clientId: number
  projectId: number
...
export function CounterpartiesTabContent({
  clientId,
  projectId,
  databases,
  recordCount,
}: CounterpartiesTabContentProps) {
  ...
  // Фильтруем базы данных, которые содержат контрагентов
  const counterpartiesDatabases = databases
    ? databases
        .filter(db => (db.counterparties_count || db.record_count || 0) > 0)
        .map(db => ({
          id: db.id,
          name: db.name,
          record_count: db.counterparties_count || db.record_count,
        }))
    : undefined
  ...
  const fullDatabases: DatabasePreviewStats[] | undefined = stats?.databases?.filter(
    (db) => db.counterparty_count > 0
  )

  const totalCounterparties = stats?.total_counterparties || recordCount || 0

  // Оценка времени обработки (примерно 1 запись в секунду)
  const estimatedTime =
    totalCounterparties > 0
      ? `~${Math.ceil(totalCounterparties / 60)} мин`
      : '~1 мин'
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <p className="text-muted-foreground text-sm">
          Просмотр исходных распарсенных записей контрагентов из всех баз данных проекта. 
          Данные загружаются напрямую из баз данных и показывают реальные записи до процесса нормализации.
          Вы можете фильтровать по базе данных, искать записи, просматривать детали и экспортировать данные.
        </p>
        <RawDatabaseRecordsTable
          dataType="counterparties"
          clientId={clientId}
          projectId={projectId}
          databases={counterpartiesDatabases}
        />
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Аналитика заполненности контрагентов</CardTitle>
          <CardDescription>
            {totalCounterparties.toLocaleString()} записей • Детальный анализ заполнения полей
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      {fullDatabases && fullDatabases.length > 0 && (
        <DatabaseTable
          ...
          filter={{ dataType: 'counterparties' }}
        />
      )}
      ...
      <NormalizationProcessPanel
        type="counterparties"
        recordCount={totalCounterparties}
